...
| **Pipeline** | Kanban CRM: columns Sourced → In Review → Offer → Closed Won. Past roles are Closed-Won deal cards (company, role, period, "deal value" = tenure); one open card, "Your Company — Senior Frontend Engineer", starts in Sourced and is draggable toward Offer. Card order and column placement persist. | `@dnd-kit` drag-and-drop, keyboard-accessible |
