...
| Layer id | Asset key(s) | Piece & story | Aspect (W:H) | Map | Status |
|---|---|---|---|---|---|
| `ch6-nave-a` | `ch6-nave-a` | The APSE rank: scalloped dome crown silhouette (die-cut alpha), aurora-rose window (radial mint→amethyst→gilt core) centered in the visible band y 0.43–0.62, flat painted treasure tiers below (mostly hidden — cheap fills), frost dusting on the crown edge | ≈3.06:1 landscape (1.90 × 0.62) | STAND | placeholder |
| `ch6-nave-b` | `ch6-nave-b` | Nave rank B: teal vault masonry, gilt arch rim ringing the aperture, coin-shelf strata lines in the wings' top band; relief = arch molding + column pair (paint continuous across the cuts) | ≈2.92:1 landscape (1.52 × 0.52) | STAND | placeholder |
| `ch6-nave-c` | `ch6-nave-c` | Nave rank C: same grammar, one bay nearer | ≈2.64:1 landscape (1.16 × 0.44) | STAND | placeholder |
| `ch6-nave-d` | `ch6-nave-d` | Rank D, the portal MOUTH (the depth-band hero): gilt arch rim, keystone step relief (order-2), T1 GOLD edge ribbon on the rim | ≈2.17:1 landscape (0.78 × 0.36) | STAND | placeholder |
