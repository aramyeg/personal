...
| `platform` | `<id>-deck`, `<id>-strut` | **OPAQUE** full-bleed rectangles; `-deck` is the tread, `-strut` the truss face (ONE sheet serves every strut on the piece — identity uvs). `-strut` is optional: supply it only when raw kraft would out-value the scene, otherwise leave it unbaked and the struts stay kraft |
...
| `ch1-keyboard` | `ch1-keyboard-board`, `ch1-keyboard-door1`, `ch1-keyboard-door2`, `ch1-keyboard-door3`, `ch1-keyboard-door4` | LIFT-THE-FLAP (the chapter's playable, G4): the inn's KEY-BOARD — a timber tavern plaque riveted flat into the right-page meadow with four numbered door leaves the reader lifts to reveal a hanging brass key behind each, save door 3 which hides the innkeeper's cat. `-board` = the plaque painted in the OPEN state (four dark recess niches, keys hanging, the cat curled in niche 3); `-door<N>` = one numbered timber door leaf (iron hinge straps at the spine edge, a ring handle at the lift edge, a brass `N` plate) | painted in SCREEN space (image-x = page-fore d, image-y = spine z), and cut to the SCREEN aspect rather than the world aspect — z is the foreshortened axis at the pinned reading camera (441 px/world in d, 244 in z), so a canvas cut to the world aspect stretches every glyph sideways (E3 wave-2 S2-1): board ≈0.93:1 (d 0.34 : z 0.66 → 512x552) · doors ≈3.3:1 (d 0.26 : z 0.142 → 512x155, number upright, font-free strokes) | FLAT (page-flat, screen-space uvs) | **art** (procedural) |
...
| `ch1-stable-vane` | `ch1-stable-vane` | Dressed weathervane overhanging the stable's roof ridge — a BRASS COCKEREL (comb, wattle, hooked beak, tail sickles) on a N-S arrow, filling the top third of the die: at slate and one sixth of the die it read as a blue nib, and took the whole stable with it into "a ledger stood tent-open with a quill in it" (S2-6b) | ≈0.58:1 portrait | FLAT | placeholder |
...
crown, an interactive tower-hoist winch (`ch3-keep-winch`), and the kept fore wall (`ch3-fringe`).

**E3 s4 ROUND-4 — THE RAVEN CITY.** Round 3's answer to a squat ring was a CANYON: two
page-rooted `stagedchain` cliffs facing each other across the gutter. The user's verdict — "still
needs to be bigger and to have some structures revised, maybe adding some asymmetry will help" —
had one cause under all three notes: a mirror has no scale, because nothing in it measures
anything else. So the mirror is RETIRED (both cliffs, the flanking mound skyline and the
strip-erected gatehouse with them) and the spread is rebuilt as ONE DIAGONAL SWEEP at dispatch
hour: a colossal crooked rookery tower (`ch3-tower`) climbing the LEFT page past the keep's crown,
a working die-cut dispatch cable (`ch3-dispatch-line`) falling across the RIGHT page with
letter-baskets riding it and a reader-pushed trolley (`ch3-dispatch-line-basket`), and a low wide
sprawl of terraced roosts (`ch3-terrace`) where the ravens sort. Asymmetry IS the composition, and
the tower is the ONE tall thing on the spread — the roosts' whole job is to make that legible.

THE TRAPEZOID CONTRACT (`stagedChainNodeU`, new in round 4) governs all three architecture pieces:
a storey no longer gets the whole sheet width, it gets the u sub-range its radial span occupies,
so a storey is a TRAPEZOID in the painting and the union of them is the die. Paint inside the
band; everything outside it is never sampled. The kinks between storeys are the crooked read, so
ledges, balconies and gantry platforms are painted flush to the boundary while the wall around
them is bitten inward — that is how a platform juts without leaving the paper. Registration
numbers (v-bands and per-node u-ranges) live in the `PIECES` entries of
`scripts/storybook/generate-art.mjs`, derived from the SAME content.ts numbers the solver folds by.

The citadel canyon behind the keep stays PAINTED aerial recession — stepped rooflines,
...
| `ch3-keep` | `ch3-keep-balcony` | THE DISPATCH DESK, seen from above — the spread's narrative centrepiece, and the one object that says “this man ran the message desk”. A blind reader found it a gorgeous gold counter at 3x and “a gold crescent with a white smear in it” at 1x, so it is composed for the ~200x110 screen px it actually projects and carries exactly five kinds of thing: a brass counter with a DARK leather writing bed (the contrast the pale ledger never had), ONE open ledger straddling the spine crease with its gutter laid deliberately ON the crease (the only feature a mid-turn hairline can appear inside without damage), heavy ink rules with blocky clerk's entries and a red money column per page, TWO quills drawn as long pale diagonals with filled vanes, TWO brass-rimmed inkwells with a specular catch, THREE wax-sealed letters, one candle seen from directly above (the reason the deck is the brightest warm island on the tier), and a balustered brass front rail carrying a small engraved POST nameplate struck clear of the crease | 910:490 = 1.857:1 (2*halfW 0.52 : z-span 0.28, exactly 13/7 — the old ≈1.7:1 predated the E1.5 grow of the balcony and was never re-derived) | TOPDOWN (split across the spine crease, art-u 0.5 at the seam; image top = the facade edge, image bottom = the front rail) | **art** (procedural) |
...
| `ch3-keep-winch` | `ch3-keep-winch-disc` | THE CRANK WHEEL, and the answer to the blind reader's first two findings (“the working winch carries no label of any kind” and “the only labelled disc on the spread is the other one”, which together read as a wiring error). A heavy brass capstan: 8 tapered spokes over a night ground, a knurled rim with every fourth cut struck deep, a riveted hub with a square drive socket, a thumb grip lobe protruding past the rim (the one cue that a hand turns this, and the reader's own read-out of how far they have wound), a curved direction arrow on each flank of the rim and a filed stop mark where the wind ends. THE LABEL is a riveted brass MAKER'S PLATE struck HOIST across the wheel's face, at two opposite stations. Round 1 engraved the word into the rim band at 9.25% cap height and it read as a tangle of dark scoring in the board capture: a page-flat disc is foreshortened to **140x74** screen px at rest (145x90 at full wind), so the vertical is squashed to ~0.53 and nothing sized off the art's own diameter keeps its height. The plate is therefore sized against that measurement — cap height 11% of the diameter, word 53.6% of it, strokes at 20% of cap height, near-black on GOLD_LIT rather than gold-on-gold, and the atlas region raised 300→460px — so it survives as a plate with writing on it even where individual letters stop resolving. Two plates because the shown wind is 290deg and no single orientation stays upright; the plates cost the outer reach of the four spokes they cross and carry a drop shadow so they read as bolted ON the face | 640:640 = 1:1 square (side 2*discR 0.26) | DISC (spins with the reader's drag — nothing on it may mean anything at only one rotation) | **art** (procedural) |
| `ch3-keep-winch` | `ch3-keep-winch-mast` | THE SIGNAL MAST (Wave-2 S4-4) — the STATIC post the paddle pivots on, standing on the loft lid at the arm's own z and running up the crown's back to the pivot. A blind reader read the hoisted paddle as “a detached prop that escaped its parent”; this is the parent, and the ROPE running the mast's whole length is the answer to “no rope, mast or cable connecting it to anything”. A slate lattice of two uprights with unevenly-bayed X-bracing, a bracketed iron foot collar with knee braces at the image bottom (so the post visibly stands ON something), a brass pulley head with a dark sheave at the top, and the pale laid rope with whipping ticks running the full length between them. It projects ~12 screen px wide, so nothing on it is thinner than ~8 art px — one screen pixel costs 6.7 art px here, and a stroke under that is a stroke nobody ever sees | 80:1032 = 0.0775:1 tall strip (2*armHalfW 0.039 : baseX - (mastFootX + MAST_FOOT_SEAM) = 0.5032; the inherited ≈0.30:1 was the semaphore's aspect copied by mistake) | STAND (art-v 0 = FOOT at the image bottom, 1 = pulley HEAD at the top) | **art** (procedural) |
| `ch3-keep-winch` | `ch3-keep-winch-semaphore` | THE SIGNAL FLAG — the LAST output in the winch's stagger, so it is the “dispatch open” cue the reader's crank ends on, and “is it recognisable as a flag going up?” is the only question it has to answer at ~12x40 screen px. A brass pivot collar with a dark axle at the image bottom (painted to continue straight out of the mast's pulley head), a short slate shaft, then a gold PENNANT with a swallowtail notch cut into its free end — the notch is the whole silhouette read, since a rectangle at this size is exactly the grey post the reader complained about — one saturated SEAL_RED band, a lamplit hoist edge, and a symmetric spread-wing raven device (symmetric because art-u runs toward reader-LEFT, so this piece prints mirrored against the screen and a handed mark would print backwards) | 200:660 = 0.303:1 (2*armHalfW 0.039 : armLen 0.1287, exactly 10/33) | STAND (art-v 0 = PIVOT at the image bottom, 1 = TIP at the top) | **art** (procedural) |
| `ch3-keep-winch` | `ch3-keep-winch-iris` | THE DISPATCH BOARD — the object the chapter's own narration is about (“he was set over the great dispatch boards”) and the payoff of the reader's first turn of the crank; ONE board printed on all four shutters. The blind verdict on its placeholder was “four featureless grey slabs poking out of a roof — at 1x they read as construction scaffolding”, so this is painted to be unmistakably PAPERWORK at the ~40-56 screen px it projects: a walnut-oak board with a batten frame and four iron hinge straps with pintle knuckles down the hinged edge, SEVEN pinned route slips at ~15% of the board's short side (each with ruled ink strokes and a brass pin, two of them wax-sealed in SEAL_RED), a chalked POST legend on a brass-framed slate let into the world-bottom rail, a raven a quarter of the board tall perched on the upper batten, and two soft roost-glow pools raking out of the loft mouth | 630:625 = 1.008:1 ((IRIS_R_HI - IRIS_R_LO) * loft height = 0.1008 : bladeLen 0.10, exactly 126/125) | STAND (hinged flap; art-u runs UP THE WALL along the hinge = image x, art-v OUT from the hinge, so the hinged edge is the image BOTTOM and the whole board is painted a quarter turn over) | **art** (procedural) |
| `ch3-keep-winch` | `ch3-keep-winch-counterweight` | THE IRON SASH WEIGHT descending the belfry mouth. Deliberately quiet — the blind reader never noticed it, which for a counterweight inside an opening is close to correct, so it is honest rather than made loud: a cast-iron block with ONE casting seam (two seams plus a waist rib read as a WINDOW at 20x31 screen px), a ground top bevel as its single bright note, a lamplit near arris, a lifting eye symmetric about the crease, and three heavy chain links struck in LIT iron because ink-on-ink chain vanished against the darkest opening on the keep | 405:640 = 0.6325:1 (2*CW_BW 0.0506 : 2*CW_BH 0.08) | STAND (split across the loft cap crease, art-u 0.5 at the crease; art-v 0 = the block's bottom) | **art** (procedural) |
| `ch3-tower` | `ch3-tower` | THE CROOKED COLOSSUS — ONE continuous portrait painting of a 4-storey crooked rookery tower on the LEFT page, sliced by the mesh into per-storey v-bands AND per-storey u sub-ranges (the trapezoid mapping). UNEVEN storeys: tall buttressed foot (3 monumental blazing gate arches), squat gantry belt kicked outboard (jutting timber platforms with lamps), tall belfry storey pulled back inboard (4-bay arcade), short leaning crown (dense dovecote ranks under a crenellated skyline with two glazed lantern posts). Die-cut: crown skyline, a linked raven rank on the parapet, ravens wheeling in the sky it leaves, and the GANTRY ARM + first stretch of dispatch cable leaving toward the gutter. u = 0 is the inboard (gutter) edge and the lit flank; near-black slate stone, blazing amber portals, hairline raw-paper rim | 295:1024 ≈ 0.288:1 (radial extent 0.301 : chain 1.045) | STAGED CHAIN (trapezoid, merged mesh, v-banded per storey) | **art** (procedural) |
| `ch3-dispatch-line` | `ch3-dispatch-line` | THE DIE-CUT CABLE PANEL — a sheet that has to read as a WIRE IN THE AIR, so ~97% of its alpha is cut away. Carries ONLY: the sagging cable polyline (the same 9 (u,v) nodes content.ts hands the solver), a trussed die-cut mast + pulley at each end, three hanging wicker letter-baskets with lit amber lanterns at arc s = 0.20/0.47/0.72, and three short raven ranks rotated onto the wire's own slope. Renders at alphaTest 0.1 over a black page, so every glow is a BOUNDED stepped bloom (a soft halo prints as a grey disc) and the wire ravens are slate, not ink. ROUND 4b: taller panel, shallower sag (v 0.965→0.48) so the wire clears the roofline over its whole run, and every weight quoted as a FRACTION of the panel — the board capture projects this sheet into ~139x253 screen px, where r4a's 4.6px rope landed at 1.2 screen px and read as a scratch | 516:1024 ≈ 0.504:1 (w 0.315 : chain 0.625) | STAGED CHAIN (2 stages, die-cut alpha; v split at 0.5760) | **art** (procedural) |
| `ch3-dispatch-line` | `ch3-dispatch-line-basket` | THE READER'S TROLLEY — the in-plane rider pushed down the wire (drive channel `~send`, release HOLDS), and the one thing on this spread a hand operates. The quad is CENTRED on the cable, so the grooved brass pulley WHEEL sits at the image's centre with the stirrup above it and the wicker pannier below: two cream envelopes, the brightest lantern on the wire with a bloom behind it (that is what makes it findable on a black page), and a brass PULL-RING on a short cord under the pannier — the diegetic "take hold of me", in the same brass as the lanterns and the winch crank rather than a grey UI affordance. Drawn to fill the cell and read at 29 x 43 px at 1600x900 with no zoom | 1:1 square | RIDER (in-plane quad, centred on the cable) | **art** (procedural) |
| `ch3-terrace` | `ch3-terrace` | THE TERRACED ROOSTS — the RIGHT page's answer to a colossus and the piece that kills the mirror by being its OPPOSITE: a low, wide, five-storey sprawl of roof after roof stepping back. Grammar is strictly horizontal — party pilasters dividing each terrace into houses, TWO long ranks of small square-headed warm roost windows per storey (never arches), dormer roost-boxes on every roof plane, deep eave shadow + lit ridge on every fold, ravens landing on the ridge below, a lamplit road with lantern posts entering at the foot (inherited from the retired yard wall), and a LOW landing mast at the outboard end where the cable comes down. Lit chance falls as it climbs — that is the aerial recession. ROUND 4b re-proportions it WIDER THAN TALL (the chain shortened so the cable could fly over it, which is what a "low sprawl" needed all along) and breaks the r4a grid: houses and their window bays are UNEVEN seeded splits each sitting a little high or low on its own string course, two storeys carry a GABLE END with a hoist beam and a lit loft door, two carry a JETTIED upper floor on corbels, chimney stacks stand off every ridge, and a house may be SHUTTERED outright so dark cells clump among the warm ones | 1024:991 ≈ 1.033:1 (radial extent 0.31 : chain 0.30) | STAGED CHAIN (trapezoid, merged mesh, v-banded per storey) | **art** (procedural) |
...
*Retired in the E3 s5 polish round: the gold-hoard BRIDGE platform and its deck painting. After the range rebuild the deck never crested anything — only its strut rig showed, silhouetted in the notch above the dragon — and the one seating that hides the rig hides the whole piece behind the hero. The retirement note in `content.ts` carries the measurements.*

| Layer id | Asset key(s) | Piece & story | Aspect (W:H) | Map | Status |
|---|---|---|---|---|---|
| `ch4-range` | `ch4-range` | E3 NOCTURNE MASSIF (mfoldrange, family #21): FOUR dune ranks + two valley-floor gusset strips on ONE 1024×1024 atlas — rows top→bottom r4 380px / r3 270px / r2 190px / r1 136px / gusset-left 24px / gusset-right 24px (popup-mfoldrange.ts `MFOLD_ATLAS_ROWS`). VAULT_NIGHT palette: violet-shadowed lee faces, gold-lit windward faces; r4's upper 40% is night sky with painted heat-shimmer columns, haze wisps die-cut into r4/r2 top edges; r3 silhouette DIPS ≤0.36 inside \|x\|<0.45 (the notch framing the dragon); r2 carries the printed micro-caravan on its lit face | 1:1 atlas (row bands per rank) | ATLAS-ROWS | **art** (procedural) |
| `ch4-hero` | `ch4-hero` | THE MONEY SHOT: the vault-dragon coiled on the round vault door, GOLD-FOIL facet treatment (seeded 4-stop gradients, ~10% thin-film teal/magenta, sparkles) — the one glowing thing in a dark spread | ≈1.28:1 landscape | STAND | **art** (procedural) |
| `ch4-aureole` | `ch4-aureole` | THE RISING COIL (E3 W2 S5-4; was the guilloché vault ring, which a blind reader read as "a separate donut prop, not part of the dragon"): a loop of the dragon itself rising off the coil that rings the vault door — same hide, same gold belly plates, same dorsal sails, same house rim as `ch4-hero`'s coil. Its footprint in the hero canvas is derived (centre art 778.7, 167.7; radii 175.3 x 178.1 px), and the hero's own cut edges are BROKEN inside that ellipse so no rim draws through the piece lying on it. | ≈1.13:1 near-square | FLAT | **art** (procedural) |
| `ch4-coins` | `ch4-coins` | Gold coins spilling off the dragon's own fold | ≈1.0:1 square | STAND — **ROTATED 180** | **art** |
| `ch4-chest` | `ch4-chest-front`, `ch4-chest-back`, `ch4-chest-side` (**no `-top`** — `roof:'open'`, hollow, no lid) | THE BANK'S COUNTER IN THE DUNES (E3 W2 S5-6; was a strongchest, and the blind reader's "least-finished-looking solid... a smudge under the dragon"). Painted for its 115x49 screen px in three bands only: a ruled gold SIGNBOARD with a struck token at each end, an arched teller window with a grille, a lamp and the clerk's silhouette, and a gold-capped counter with coin stacks and a ledger. The BACK cap carries the same three bands — it is the face that clears the passage of glass standing in front of the counter. | side ≈1.0:1 · front/back ≈2.0:1 | FACE | **art** (procedural) |
| `ch4-chest-lid` | `ch4-chest-lid` | Dressed propped-open lid silhouette off the chest's side wall | ≈0.93:1 near-square | FLAT | **art** (procedural) |
| `ch4-chest-spill` | `ch4-chest-spill` | Dressed heaped gold on the chest's front cap | ≈1.33:1 landscape | FLAT | **art** (procedural) |
| `ch4-goldpile` | `ch4-goldpile-face`, `ch4-goldpile-tab` | MOUND tab piece: loose gold rising as the spread blooms, tab creeps out the fore edge. `-tab` is the PULL TONGUE (E3 W2 S5-1): a struck gold plate with a stitched root at the slit, one ember arrow aimed out of the page, and the bank's dune token — the same shared `pullTongue` painter as `ch4-dissolve-tab`, so the spread's two handles speak one vocabulary. Tongue axes: image u = the tab's width along the spine, image v = the page-fore axis with v=1 the OUTER tip (SVG top). | face u:v ≈0.54:1 (portrait unfolded strip) — bands: `slopeIn` v∈[0,0.5], `slopeOut` v∈[0.5,1] · tab ≈1:1.007 (tabW 0.24 : lip+slide 0.2417 at rest) | UNFOLD | **art** (procedural) |
| `ch4-dissolve` | `ch4-dissolve-dunes`, `ch4-dissolve-gold`, `ch4-dissolve-tab` | PULL-TAB DISSOLVE (VAULT_NIGHT regrade: moonlit violet dunes face <-> foil-ramp gold face, both carrying the engraved brass slot plate + ember chevrons + PULL; `-tab` is the PULL TONGUE itself — shared `pullTongue` painter with the gold pile's, struck with the glazed arch of the passage it opens, kraft-grip fallback when absent) (Birmingham 92/93; the book's only paper CROSSFADE): a page-flat rack of 6 venetian SLATS on the LEFT-page open sand field (the mirror of the right-page goldpile tab). Pull the tab and rolling DUNES with a distant camel-train FLIP through the edge-on "blinds close" to the dragon's GOLD hoard — the two paintings share ONE composition (matching ridgelines, sun, and caravan station) so the flip reads as a transmutation. `-dunes` is the up-face at rest (tau=0), `-gold` the under-face revealed at tau=PI. Painted in SCREEN space (image-x = the page-fore axis d = the 6-slat stack; image-y = the spine axis z, SVG top = far sky, SVG bottom = near foreground), sliced into 6 vertical strips. At rest the shut rack LEAKS: a warm seam painted on each of the six slat boundaries (u = k/6, the exact `slatUvs` lines) plus a foil halo round the card, so the closed state promises the gold behind it (S5-7). | dunes/gold ≈1.18:1 each (d 0.52 : z 0.44) · tab ≈1:0.31 at rest (tabW 0.26 : lip+tongue 0.08), 512x220 splitting the difference with the latched-gold end | SLAT-FLIP (screen-space) | **art** (procedural) |
| `ch4-frieze` | `ch4-frieze` | THE PASSAGE OF GLASS (E3 W2 S5-3/S5-4 repaint of the caravan frieze) — the chapter's title object, in elevation across the whole spread: stone kerb, gold mullions with capitals, arched translucent glazing (fill-opacity 0.3, so the sand, the counter and the dragon read THROUGH it), a gold top rail with finials, lamps hung inside, and the caravan + its hooded customers walking WITHIN it. The gallery ends at u=0.21 in a lamplit PORTAL under a signboard; only the 20%-tall kerb continues past it as a paved approach, which is how the pack §4.2 sightline law (silhouette ≤0.06 world = 37.5% of h in x∈[−0.75,−0.44] = u≤0.207) survives the rebuild — the dissolve placard still reads through. | ≈9.4:1 very wide sliver | STAND | **art** (procedural) |
...
| `ch5-throng` | `ch5-throng` | THE HERO + the spread's playable: linked rank of SIX IDENTICAL market stalls (crate, two walnut posts, striped awning + scalloped valance, cream lift-lip with a ⟡ on the free edge). Ships lying FLAT on the near apron — the master pattern printed on the page — and the reader's drag raises it. All six identical: the repetition is the chapter | ≈3:1 very wide | STAND | procedural |
...
| `ch5-raise-stall` | `ch5-raise-stall-face`, `ch5-raise-stall-tab` | The celebrated playable: retained TABLE tab piece re-themed — legs as stall posts, deck a striped awning mid-raise carrying the woodcut RAISE-A-STALL cartouche. The whole erected body is a grab surface, not just the tab (S6-1). `-tab` is the PAINTED pull tab (the optional tabpiece tab key): u across the tab's width, v along the pull — v=0 at the slit in the page fore edge, v=1 at the outer finger end (punched hole + ⟡ + outward chevron), same teal/cream stripe cadence as the deck so it reads as the same stock | face u:v ≈0.5:1 (portrait unfolded strip) — bands: `legIn` v∈[0,0.32], *deck* v∈[0.32,0.68], `legOut` v∈[0.68,1] · tab u:v ≈0.5:1 portrait | UNFOLD | procedural |
...
| `ch6-clerk` | `ch6-clerk` | The intimate counterweight: a kneeling clerk over an open ledger, one candle — midnight coat, candle-gold face; painted both sides; gaze 3/4 toward the coffer | ≈0.81:1 portrait (0.21 × 0.26) | STAND | **art** (procedural) |
...
| `ch6-crest` | `ch6-crest` | The treasury's NIGHT-LAMP standing ON the strongbox lid (rider recursion) — a hooded brass lantern with a hot core; the spread's idle accent (idle-tagged glint — the flame breathing). Was a rampant griffin, which at ~45 screen px was gold filigree no reader could name | ≈1.13:1 near-square | STAND | **art** (procedural) |
| `ch6-strongbox-seal` | `ch6-strongbox-seal` | The strongbox's GLAZED INSPECTION PANE on its front cap — a small pane of the treasury's own glass with gold showing behind it; idle-tagged glint. The id keeps its `-seal` name: the violet wax seal it used to carry moved to the counting wheel's thumb lobe (a press-me cue on a piece that cannot move is the defect S7-1 exists to kill), and renaming a baked piece churns five files | ≈1.0:1 square | FLAT | **art** (procedural) |
...
| `ch6-steps` | `ch6-steps-deck`, `ch6-steps-strut` | The entrance DAIS — a low bridge deck the painted gold processional path climbs into rank D's portal. `-strut` paints the two mirror-bridge trusses as a carved stone flight with gilt nosings: raw kraft made them the brightest mass under the arches, which read as scaffolding rather than as the step the path climbs | deck ≈2.08:1 wide (0.25 × 0.12) · strut 2:1 wide | TOPDOWN (deck) · FLAT (strut, treads across the sheet) | **art** (procedural) |
| `ch6-assay` | `ch6-assay-dial` | THE COUNTING WHEEL (volvelle — the spread's HEADLINE play, S7-1): a gilt wheel riveted flat into the treasury floor, EIGHT STRONGROOMS on 45° sectors (vaults I–VIII: bar gold, coin, plate, jewels, ledgers, crown, keys, the aurora vault). Every sector differs on three axes at once — a COUNT of gold bars on a shared pale shelf, a bold DEVICE silhouette, and the wedge's FIELD tint — so one detent visibly changes what all three windows frame. The violet WAX SEAL rides the thumb lobe that juts past the rim: the grip, and the object the blind reader tried to press | 1:1 square (disc, radius 0.13 → canvas 0.26×0.26) | DISC | **art** (procedural) |
| `ch6-assay` | `ch6-assay-card` | The static GLASS FACEPLATE over the wheel — a round disc of the treasury's own curtain wall (gold mullion grid, teal panes, aurora glints) with THREE ARCHED VITRINES die-cut clean through it at 45/90/135° over the read band 0.38–0.86R, each ringed by a gold arch bezel with a keystone tick; a brass tag engraved TURN with a manicule pointing into it; a plain gold hub rivet (the layer discards any twist begun inside 0.25R, so nothing invites a hand there). RIGHT-PAGE identity uvs — author in screen space. Registration pair with the dial: shared basis R = w*0.46875 | 1:1 square (disc, matches the dial) | DISC (windowed) | **art** (procedural) |
...
| `satchel-table` | `satchel-table-deck`, `satchel-table-strut` | The map table: off-center, non-mirrored BRIDGE deck, standing on the right page | ≈1.05:1 near-square | TOPDOWN | placeholder |
