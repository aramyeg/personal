...
**Rotated 180° (6 shipped pieces + 2 fan members):** `ch1-dormer`, `ch2-bee-c`,
`ch4-coins`, `ch6-door`, `ch6-banner`, `title-crest`,
`satchel-burst-m0`, `satchel-burst-m1`. (`end-raven` is now a kinetic arm — upright,
like `ch2-windmill`; the ch5 lantern pair retired in the E3 s6 rebuild — the
lanterns are now die-cut strings on the stall-arc plates.)
