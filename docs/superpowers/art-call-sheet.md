...
letter-baskets riding it and one the reader pushes (`ch3-dispatch-line-basket`), and a low wide
...
| `ch3-dispatch-line` | `ch3-dispatch-line-basket` | THE READER'S BASKET — the in-plane rider pushed down the wire (drive channel `~send`, release HOLDS). The quad hangs it BELOW the cable, so the hook and yoke sit at the very TOP edge of the image and the pannier fills the rest: warm brown wicker (the chapter's one basketwork material), two cream envelopes poking out, a lit lantern slung off the yoke | 1:1 square, drawn to fill the cell | RIDER (in-plane quad) | **art** (procedural) |
