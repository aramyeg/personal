...
| `ch3-keep-winch` | `ch3-keep-winch-mast` | THE SIGNAL MAST (Wave-2 S4-4) — the STATIC lattice post the paddle pivots on, standing on the loft lid at the arm's own z and running up the crown's back to the pivot. A blind reader read the hoisted paddle as "a detached prop that escaped its parent"; this is the parent. Slate lattice with a brass pulley head, a rope running the whole length, and a bracketed foot collar; drawn very narrow (12 screen px) so it reads as rigging, not a wall | ≈0.30:1 tall strip | STAND | procedural |
