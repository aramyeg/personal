...
| `ch3-keep-winch` | `ch3-keep-winch-disc` | THE CRANK WHEEL, and the answer to the blind reader's first two findings (“the working winch carries no label of any kind” and “the only labelled disc on the spread is the other one”, which together read as a wiring error). A heavy brass capstan: 8 tapered spokes over a night ground so they read as spokes at any size, a knurled rim with every fourth cut struck deep, a riveted hub with a square drive socket, and a thumb grip lobe protruding past the rim (the one cue that a hand turns this, and the reader's own read-out of how far they have wound). The label is in-world rather than a UI sticker: the rim band is ENGRAVED HOIST at cap height ~9% of the disc diameter, doubled (a dark cut under a bright ghost) and STRUCK TWICE at opposite stations so the legend is never all upside down through the 368deg wind, with a curved direction arrow in each of the two remaining band gaps and a filed stop mark where the wind ends | 640:640 = 1:1 square (side 2*discR 0.26) | DISC (spins with the reader's drag — nothing on it may mean anything at only one rotation) | **art** (procedural) |
