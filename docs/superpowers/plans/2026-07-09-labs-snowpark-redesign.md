...
- **Natural detach (AMENDED during execution — the original per-frame ballistic-gap rule scales with dt² and can never fire at 120 fps):** frame-rate-independent curvature rule. `slope.ts` gains `slopeCurvature(x)` — the exact analytic second derivative of `slopeY` (`-(amp/λ²)·sin(...)` per wave term), tested against the numeric second derivative. Detach when `slopeCurvature(x) * vx² > PHYS.DETACH_G` where `vx = cos(angle) * speed` and `DETACH_G = 220` (an arcade threshold well below GRAVITY — real physics never detaches on these gentle rollers; tunable at GATE A). On detach: enter air with `vx = cos(angle)*speed`, `vy = sin(angle)*speed`, `justLaunched = true`, coyote window opens (`coyoteT = COYOTE_S`). `DETACH_EPS` is removed from PHYS; `KICKER_BOOST: 1.45` and `IMPACT_DECAY: 4` are added (they were prose-only before).
