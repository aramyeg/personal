...
## Game-Specific TDD (Portfolio Project)

### Game Testing Priorities

1. **Skill State Immutability** (CRITICAL)
2. Physics with delta time
3. Collision detection
4. State machine transitions
5. Level generation determinism

### Game Test Patterns

```typescript
// 1. Immutability Test (MANDATORY for all skill code)
describe('Skill Immutability', () => {
  it('does not mutate original state', () => {
    const original = createInitialSkillState()
    Object.freeze(original)
    Object.freeze(original.doubleJump)

    expect(() => {
      applySkillPhysics(player, original, new Set([' ']), 1)
    }).not.toThrow()
  })
})
...
npm run test:coverage -- --testPathPattern=lib/game

# Watch during development
npm test -- --watch --testPathPattern=game
```

### Game Test File Structure
...
**Remember**: Tests are not optional. They are the safety net that enables confident refactoring, rapid development, and production reliability. For game code, immutability tests are CRITICAL to prevent runtime errors.
