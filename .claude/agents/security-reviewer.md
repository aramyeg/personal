...
### 3. hishcore Security Checks

**CRITICAL - App Handles Personal Memories:**

```
Privacy Security:
- [ ] All memories encrypted at rest (Supabase default)
- [ ] No memory content logged to console
- [ ] No PII sent to analytics
- [ ] User data isolated via RLS
- [ ] Session tokens not exposed
- [ ] No memory leakage in error messages

Authentication Security (Supabase Auth):
- [ ] Email/password auth properly implemented
- [ ] Session tokens validated on data access
- [ ] signOut clears all local storage
- [ ] No authentication bypass paths
- [ ] Rate limiting on auth endpoints (Supabase default)
- [ ] Email verification enabled

Database Security (Supabase):
- [ ] Row Level Security (RLS) enabled on entries table
- [ ] RLS policy: auth.uid() = user_id
- [ ] No direct database access bypassing RLS
- [ ] No SQL injection via user input
- [ ] No PII in application logs
- [ ] user_id validated before insert/update

Storage Security:
- [ ] AsyncStorage used for non-sensitive session data
- [ ] No secrets stored in AsyncStorage
- [ ] Supabase session properly encrypted
...
- [ ] WebView (if used) has proper restrictions

API Security:
- [ ] All data operations require authentication
- [ ] Input validation on body text (max 1000 chars)
