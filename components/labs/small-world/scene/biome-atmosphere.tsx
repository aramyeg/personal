...
const crossfade = (from: THREE.Color, to: THREE.Color, mix: number): THREE.Color =>
  SCRATCH.copy(from).lerp(to, mix)
...
export function BiomeAtmosphere({ journeyRef }: { journeyRef: JourneyRef }) {
  ...
  useFrame(() => {
    ...
    if (key.current) {
      key.current.color
        .copy(BASE_KEY)
        .lerp(crossfade(MOOD_CAST[prev], MOOD_CAST[b.chapter], b.mix), b.lightMix)
    }
    if (ambient.current) {
      ambient.current.color
        .copy(BASE_AMBIENT)
        .lerp(crossfade(MOOD_CAST[prev], MOOD_CAST[b.chapter], b.mix), b.lightMix * AMBIENT_FOLLOW)
    }
  })
