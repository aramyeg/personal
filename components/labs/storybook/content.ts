...
// Chapter I — E4 "PAINTED" s2 REBUILD: THE INN OF A HUNDRED KEYS as ONE
// GRAND STRUCTURE (bench .superpowers/sdd/bench/e4s2-reach.mjs).
//
// The E3 stage-set is RETIRED WHOLE. Twelve entries — three width-graded
// gutter-spanning valley folds (mountain / inn row / gate), the dormer, the
// key-sign and the brass key riding the inn crease, the lift-flap key-board,
// the stable box with its two dresses, and the fore-edge wall — became, at the
// reading camera, a stack of flat cards with a small brown box in front of
// them. Nothing in it was volumetric, nothing overhung anything, and every
// piece was the same size. Six entries replace them, on the E-P2 law (fewer,
// larger, distinct structures; variety lives at the BOOK level):
//
//   1. ch1-inn    keepstack — the gutter-straddling inn, the ONLY tall thing.
//   2. ch1-wing-l stagedchain — the tall yard wall, left, forward, raked.
//   3. ch1-wing-r stagedchain — its SHORTER, fewer-storeyed twin, right.
//   4. ch1-rank   stripflap — the innkeeper's family, rising in the archway.
//   5. ch1-yard   dissolve — the courtyard floor, day flipping to lamplight.
//
// (five layers; the sixth item on the fate list is the chapter entry's own
// `hero`, retargeted from the retired ch1-inn-row to ch1-inn.)
//
// THE COMPOSITION LAW THIS SCENE IS BUILT ON: exactly ONE element above
// y = 0.55 and everything else at or below 0.35, so the inn reads as a
// building and not as the tallest of five cards.
const CH1_LAYERS: readonly SceneLayer[] = [
  // ---------------------------------------------------------------------
  // THE INN — one keepstack entry expanding to two stacked box poses plus a
  // jutting balcony and a fan spire. Read popup-keepstack.ts for the paper:
  // a flat-lidded box fold's lid IS a miniature spread hinged at exactly the
  // page dihedral, so a story seated on the lid below is the same box solve
  // with its bisector-x offset by the lower story's height. One rigid chain,
  // one driver (beta), dead flat at close for free.
  //
  // THE BINDING CONSTRAINT IS NOT HEIGHT, IT IS FOLD-FLAT REACH. At book
  // close the whole stack folds flat ALONG the page, so every corner's
  // bisector-x must stay inside PAGE_W = 1.15. Per family the closed reach is
  //   stories   sum(H below) + a + H
  //   plates    sum(H below) + a + plate.height      (plate.height may exceed H)
  //   balcony   H_hall + halfW
  //   spire     sum(H) + the flat-folded member run
  // and it is the GUEST PLATE and the SPIRE that bind here, not the walls.
  // Measured on the shipped config (bench R1/R2): worst reach 1.0455 against
  // 1.15, fold-flat residual 0.0 exactly, apex 0.9407 world Y at the reading
  // pose (0.9407 structural crown; 1.0273 including the raven finial).
  //
  // THE HALL HAS NO FRONT CAP, AND THAT IS THE POINT (capFront: false).
  // Verified in solveBoxPose/keepStackFacadePlate: the facade plate is solved
  // from `a`, `z1` and `plate` alone and never consults capFront, so a
  // cap-less tier still mounts its plate in the (now empty) cap plane — both
  // the merged and the per-mesh renderer draw it. What the missing cap buys is
  // a SEE-THROUGH: the plate's die-cut carriage arch looks into the box, where
  // the backbone slab and the far wall stand in interior shadow, and the
  // innkeeper's family (ch1-rank) rises inside that recess.
  {
    id: 'ch1-inn', kind: 'hero', role: 'story', mech: 'keepstack',
    stories: [
      // GROUND — the coaching hall. capFront:false (see above), so the ONLY
      // thing printed in the front plane is the die-cut plate; the structural
      // cap height and the plate height are therefore equal by construction
      // (phase-1 "structure fits inside art": any plate shorter than its tier
      // leaves a bare horizontal slot, and with no cap behind it that slot is a
      // hole straight through the building). Plate width 1.60 against a cap
      // width of 0.84 — a deliberate 0.38-per-side LATERAL OVERHANG, the bailey
      // -wing idiom: coplanar with the cap, so it costs exactly zero off-plane
      // reach and zero fold-flat risk, and it is what makes the inn read
      // edge-to-edge instead of as a tower. Aspect locked at 4.000.
      { key: 'hall', a: 0.42, height: 0.4, z0: -0.3, z1: 0.3, roof: 'flat', capFront: false, capBack: true, plate: { width: 1.6, height: 0.4 } },
      // UPPER — the guest floor, telescoped inward (a 0.28 <= 0.42) and nested
    ...
    // for free (no new DOF). It starts at z 0.32 — in front of the guest wall
    // (z1 0.22) and in front of the hall's own facade plane (z ~0.32) — so
    // nothing occludes it, and it juts 0.28 further toward the reader over the
    // carriage arch. Deck aspect 2*halfW/(z1-z0) = 2.143.
    balcony: { halfW: 0.3, z0: 0.32, z1: 0.6 },
    // THE FAN SPIRE — three v-fold members sharing ONE apex, seated on the
    // guest lid (apex on the lid seam at bisector-x = sum(H) = 0.64). Solved as
    // a page fan translated up the bisector, so it is bit-identical physics and
    // folds dead flat for free. ASYMMETRIC by construction: the members run
    // laid-back broad flank -> steep narrow peak, at three different crease
    // elevations, and each crease sits OFF-MIDDLE in its own sheet (creaseU).
    // apexZ 0.05 puts the whole crown a little fore of the guest centre,
    // toward the reader.
    //
    // THE ASYMMETRY IS creaseU AND NOT skewDeg, and that is a measurement, not
    // a preference: a skewed member takes the general two-cone branch of the
    // bisector solve, whose acos/atan2 pair leaves ~8e-8 of off-page residual
    // at beta = 0 (bench R1, swept per family). Fold-flat in this book is an
    // identity, not a tolerance. creaseU moves the crease off-middle through
    // the SYMMETRIC closed form and folds flat to 1e-32 — the same off-centre
    // ridge, exactly flat. Measured: skew 8.00e-8, creaseU 6.16e-33.
    spire: {
      apexZ: 0.05,
      vDir: 1,
      members: [
        // flank — the broad laid-back sail (crease elevation ~42deg).
        { phiDeg: 34, rhoDeg: 52, creaseU: 0.62, width: 0.28, height: 0.24 },
        // mid
        { phiDeg: 41, rhoDeg: 63, creaseU: 0.58, width: 0.2, height: 0.28 },
        // peak — steepest and narrowest, and the raven's seat.
        { phiDeg: 48, rhoDeg: 76, creaseU: 0.64, width: 0.14, height: 0.3 },
      ],
      // The weathervane raven: a coplanar extension of the peak member past its
      // ridge tip. Coplanar with a folding panel => zero off-plane reach. Its
      // run up the crease is what sets the scene's TOPMOST point, and it is
      // sized so the whole inn tops out at 1.000 world exactly (bench R5).
      raven: { finialH: 0.075 },
    },
  },
  // ---------------------------------------------------------------------
  // THE YARD WALLS — two page-rooted staged chains, forward of the inn and
  // outboard of its walls (F >= 0.48 against the hall glue at 0.42), raked back
  // toward the inn so they funnel the eye into the carriage arch. NO RISERS and
  // no platform: a strut scaffold silhouetted above a low hero is a known
  // failure class here, and these are low by law (apex <= 0.35 — exactly one
  // thing on this page is tall).
  //
  // ASYMMETRIC ON PURPOSE: left is a THREE-storey wall, right a TWO-storey one
  // ~26% shorter. Their cams also saturate at different dihedrals, which is
  // what gives the page turn its staggered erection beats (right wall lands
  // first, left wall second, the inn's spire last at the rest bloom).
  {
    id: 'ch1-wing-l', kind: 'midground', role: 'scenery', mech: 'stagedchain',
    side: 'left', style: 'ribbon', F: 0.5, w: 0.24, zc: 0.6, rootDeg: 70, safe: 0.95, camRestDeg: 158,
    stages: [
      { h: 0.14, relDeg: 0, rTop: 0.52, wTop: 0.22 },
      { h: 0.12, relDeg: 5, rTop: 0.5, wTop: 0.19 },
      { h: 0.09, relDeg: 8, rTop: 0.53, wTop: 0.14 },
    ],
  },
  {
    id: 'ch1-wing-r', kind: 'midground', role: 'scenery', mech: 'stagedchain',
    side: 'right', style: 'ribbon', F: 0.48, w: 0.22, zc: 0.54, rootDeg: 74, safe: 0.95, camRestDeg: 140,
    stages: [
      { h: 0.16, relDeg: 0, rTop: 0.5, wTop: 0.2 },
      { h: 0.1, relDeg: 6, rTop: 0.49, wTop: 0.17 },
    ],
  },
  // ---------------------------------------------------------------------
  // THE WELCOME RANK — the innkeeper, spouse, child and dog die-cut as ONE
  // linked chain, hinged INSIDE the coaching hall (hingeZ 0.18, well behind the
  // facade plane at z ~0.32) and reaching out through the carriage arch onto
  // the step. The hall is 0.40 tall, so a 0.38 flap stands upright inside the
  // archway without piercing the lid, and its 0.36 width spans radial
  // 0.09..0.40 — inside the hall's own walls at 0.42.
  //
  // THE PIECE IS ON THE RIGHT PAGE BECAUSE THAT IS WHICH WAY IT LIES. `flat` is
  // cross(hinge, n), and n flips with the page: at hingeDeg 0 a right-page flap
  // lies FORE (toward the reader) and a left-page one lies AFT. The fore lie is
  // the monotone one at this camera — the aft lie's screen height peaks at
  // 56.7deg and a window straddling that peak measures ~16 px for 46deg of paper
  // (the shipped E3 rank's defect). So: right page, fore lie, and the family
  // reaches out through the arch onto the step as it is lowered.
  //
  // THREE DIALS, ALL SWEPT (bench --sweep-rank), because they trade against each
  // other and no two of them can be maximised:
  //   * restDeg buys TRAVEL and costs READABILITY. A fore-lying flap's face is
  //     exactly edge-on to a camera at its own depression angle (26.1deg here),
  //     so face-on climbs roughly linearly out of that zero: rest 38 -> 0.21,
  //     42 -> 0.23, 50 -> 0.35, and travel falls 136 -> 123 -> 98 px over the
  //     same range. Below ~34 the welcome party is face-down on the cobbles.
  //   * hingeDeg buys travel too (it trades cancelling vertical for page-fore
  //     motion on the 441 px/world axis) AND it conditions the reader's own
  //     pointer: |view . hinge| is 0.043 at hingeDeg 0 — the ill-conditioned
  //     swing plane that forced s2's cylinder opt-in — 0.348 at 20, 0.499 at 30.
  //     It costs frontality, and this piece must face the reader.
  //   * height is capped by the ceiling it stands under: the hall lid sits at
  //     0.4405 world at the reading pose, so 0.38 is the tallest family that can
  //     come fully upright inside the archway without piercing the roof.
  // SHIPPED: 0.38 tall, hingeDeg 20, window [44, 90]. Measured 116.6 px of
  // worst-vertex screen travel = 12.96% of frame height (floor 12%), face-on
  // 0.26 bowed and 0.85 upright, |view . hinge| 0.348, standing top 0.4128
  ...
  // orientation and the reading camera's best one: no foreshortening trap, both
  // end states coplanar, so the piece needs no fold-flat envelope at all.
  // State A is the yard by day; state B is the same yard at dusk with every
  // window and lantern lit — a hundred keys handed out. Slats span the FULL z
  // band, so both paintings share one framing.
  { id: 'ch1-yard', kind: 'foreground', role: 'scenery', mech: 'dissolve', side: 'left', d0: 0.46, d1: 1.06, z0: -0.32, z1: 0.52, slats: 7, stroke: 0.14, tabW: 0.26, tabTip: 0.06, turnCull: true },
]
...
export const CHAPTERS: readonly Chapter[] = [
  {
    ...
    layers: CH1_LAYERS,
    // E4 PAINTED: the retired ch1-inn-row v-fold gives the signature moment up
    // to the inn itself — a two-storey keepstack whose spire crown climbs to
    // ~0.94 world as the page opens. Family keepstack differs from the next
    // spread's swarmarc, so the hero-rotation law holds.
    hero: 'ch1-inn',
    // ONE grand structure plus its yard, not the D-series per-chapter crowd
    // template (E-P2). Same marker ch3 carries; every physics gate still binds.
    showpiece: true,
  },
  {
    spread: 3,
